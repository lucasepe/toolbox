...
func TestCustomAcronymsToCamel(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			text.ConfigureAcronym(test.acronymKey, test.acronymValue)
			if result := text.ToCamel(test.acronymKey); result != test.expected {
...
func TestCustomAcronymsToLowerCamel(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			text.ConfigureAcronym(test.acronymKey, test.acronymValue)
			if result := text.ToLowerCamel(test.acronymKey); result != test.expected {
				t.Errorf("expected custom acronym result %s, got %s", test.expected, result)
			}
		})
	}
